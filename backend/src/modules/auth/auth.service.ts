...
export const authService = {
  ...
  forgotPassword: async (email: string): Promise<void> => {
    ...
    logger.info(`Password reset token generated for user (ID: ${user.id}). Token: ${token}`);
  },
  ...
  resendVerification: async (userId: string): Promise<void> => {
    ...
    logger.info(`New email verification token generated for user (ID: ${user.id}). Token: ${token}`);
  },
};
